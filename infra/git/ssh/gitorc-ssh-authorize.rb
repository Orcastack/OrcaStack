...
fingerprint = ENV['GITORC_SSH_KEY_FINGERPRINT'].to_s
username = ENV['GITORC_SSH_USERNAME'].to_s

abort('SSH_ORIGINAL_COMMAND is required') if command.empty?
abort('GITORC_SSH_KEY_FINGERPRINT is required') if fingerprint.empty?

ruby_app_url = ENV.fetch('GITORC_RUBY_APP_URL', 'http://127.0.0.1:3000')
git_service_url = ENV.fetch('GITORC_GIT_ENGINE_URL', 'http://127.0.0.1:8081')
