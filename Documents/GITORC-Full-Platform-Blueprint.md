# GITORC Full Platform Blueprint

## Purpose

This document defines the Go-first platform architecture implemented and targeted by this repository. It treats GITORC as a React control plane, a set of Go microservices, and a private-cloud automation stack rather than as a Rails-based monolith.
...
The operator-facing UI is implemented in React with Vite under `gitorcweb/`. It presents dashboard views, onboarding flows, project and repository surfaces, security framing, documentation access, and pipeline-oriented operator workflows.

### 2. Gateway layer

The gateway is the primary application entrypoint and is implemented in Go under `gitorcapi/cmd/gitorc-gateway/` and `gitorcapi/internal/gatewayapi/`. It handles session-oriented auth flows, local and LDAP-backed identity checks, project-facing API surfaces, and aggregated overview data for the UI.

### 3. Core service layer

GITORC decomposes its platform logic into Go services rather than concentrating it inside a single application server. Current service entrypoints include:
...
| --- | --- | --- | --- | --- |
| Frontend control plane | React application | Implemented | `gitorcweb/`, `gitorcweb/package.json`, `gitorcweb/src/App.tsx` | React + Vite is the active UI stack. |
| Dashboard and operator navigation | UI for platform surfaces | Partial | `gitorcweb/src/App.tsx` | Broad platform surfaces exist; some features are still presentation-first. |
| Gateway and session entrypoint | Go API gateway | Implemented | `gitorcapi/cmd/gitorc-gateway/`, `gitorcapi/internal/gatewayapi/` | Primary control-plane entrypoint. |
| Identity and access | Local auth + LDAP-backed auth in Go | Partial | `gitorcapi/internal/gatewayapi/auth_ldap.go`, `gitorcapi/internal/gatewayapi/local_auth.go` | Identity is present, but deeper IAM services remain limited. |
| User profile and preferences | UI and gateway-backed user settings | Partial | `gitorcweb/src/App.tsx`, gateway session APIs | Present in the UI model, not yet a deep service surface. |
...
| Notifications and webhooks | Go integrations and webhook handlers | Partial | `gitorcapi/internal/platform/discord/`, `gitorcapi/internal/platform/slack/` | Outbound notification paths exist. |
| Wiki and documentation | Repository-backed docs and viewer surfaces | Partial | `docs/`, `docs/wiki/`, `gitorcweb/src/App.tsx` | Docs are strong; a dedicated wiki service is not yet present. |
| Admin workflows | Operator and approval surfaces | Partial | `gitorcweb/src/App.tsx`, signup review flows | Admin behavior exists but is not fully separated as its own backend domain. |
| Git service | Dedicated Go Git service | Implemented | `gitorcapi/cmd/gitorc-git-service/`, `infra/git/repos/` | Core Git hosting surface. |
| Review service | Dedicated Go review service | Implemented | `gitorcapi/cmd/gitorc-review-service/` | Review and approval surface. |
| CI execution | Dedicated Go CI service | Implemented | `gitorcapi/cmd/gitorc-ci-service/` | Build and pipeline execution surface. |
| CD execution | Dedicated Go CD service | Implemented | `gitorcapi/cmd/gitorc-cd-service/` | Deployment coordination surface. |
| Analytics service | Dedicated Go analytics service | Implemented | `gitorcapi/cmd/gitorc-analytics-service/` | Reporting and telemetry-oriented service boundary. |
| Security control | Dedicated Go security service and policy assets | Implemented | `gitorcapi/cmd/gitorc-secctl/`, `infra/policy/` | Security-control capability exists in service and infra layers. |
...
1. GITORC should be described as a Go-first service platform with a React control plane.
...
2. Clarify whether hardware automation and device orchestration should remain infra-driven or become dedicated Go services under `gitorcapi/cmd/`.
